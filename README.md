...
`tools/list` at `https://gateway.pipeworx.io/gdacs/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/gdacs_events \
  -H 'Content-Type: application/json' \
  -d '{"event_type":"EQ","alert_level":"Red"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/gdacs_events`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
